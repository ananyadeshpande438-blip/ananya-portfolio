        ...
        {portfolioData.education.map((edu, idx) => (
          <div key={idx} className="glass p-6 rounded-2xl border-t-4 border-t-[#F97316] hover:-translate-y-2 transition-all duration-300 shadow-sm hover:shadow-lg group flex flex-col h-full">
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="w-12 h-12 bg-white border border-[#E8E4DC] rounded-full flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                {getEducationIcon(edu.type)}
              </div>
              {edu.percentage && (
                <span className="flex items-center gap-1 text-sm font-bold text-slate-700 bg-white px-2 py-1 rounded shadow-sm border border-[#E8E4DC]">
            ...
            <span className="text-xs font-bold px-3 py-1 bg-[#FAF7F0] text-[#F97316] rounded-full uppercase tracking-wide w-fit mb-3">
              {edu.type}
            </span>
            ...
            <div className="flex items-center gap-2 text-sm text-slate-500 font-medium bg-white w-fit px-3 py-1.5 rounded-md mb-4 shadow-sm border border-[#E8E4DC] mt-auto">
              <Calendar size={14} className="text-[#F97316]" />
              {edu.duration}
            </div>
            
            {edu.description && (
              <p className="text-slate-600 text-sm leading-relaxed border-t border-[#E8E4DC]/60 pt-4 mt-4">
                {edu.description}
              </p>
            )}
          </div>
